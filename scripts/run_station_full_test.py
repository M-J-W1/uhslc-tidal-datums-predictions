...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run full diagnostics for one station.")
    parser.add_argument("--station-id", default=DEFAULT_STATION_ID)
    args = parser.parse_args()

    station_id = str(int(args.station_id)).zfill(3)
    output_root = Path("artifacts") / "full_test" / f"station{station_id}"
    ...
    md_lines = [
        f"# Station {station_id} Full Test",
        "",
